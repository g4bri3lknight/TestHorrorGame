...
// --------------------------------------------------------------------------
public class HDK_AIZombieStateMachine : HDK_AIStateMachine
{
    // Inspector Assigned
    ...
    //UI Damage Indicator
    public bool ShowDamageCanvas;
    public float distanceToShow;
    public GameObject DamageCanvas;
    public Text headDamageText;
    public Text lowerDamageText;
    public Text upperDamageText;
    ...
    protected override void Update()
    {
        ...
        if (_currentStateType != AIStateType.Dead)
        {
            if (ShowDamageCanvas)
            {
                ...
                {
                    DamageCanvas.GetComponent<CanvasGroup>().alpha += Time.deltaTime * 2f;
                }
                else
                {
                    DamageCanvas.GetComponent<CanvasGroup>().alpha -= Time.deltaTime * 2f;
                }
                int headDam = 100 - health;
                headDamageText.text = headDam.ToString("F0") + " %";
                upperDamageText.text = _upperBodyDamage.ToString("F0") + " %";
                lowerDamageText.text = _lowerBodyDamage.ToString("F0") + " %";

                if (headDam >= 70)
                {
                    headDamageText.color = Color.red;
                }
                if (_upperBodyDamage >= 70)
                {
                    upperDamageText.color = Color.red;
                }
                if (_lowerBodyDamage >= _crawlThreshold)
                {
                    lowerDamageText.color = Color.red;
                }
            }
        }
        else
        {
            DamageCanvas.GetComponent<CanvasGroup>().alpha -= Time.deltaTime * 2f;
            //MODIFICA - Comunica il nome dell'oggetto per il salvataggio/caricamento e disattiva l'oggetto
            GameObject enemy = this.transform.parent.gameObject;
            GetComponent<OnDestroyNotify>().OnDestroy(enemy);
            this.enabled = false;
        }
    ...
    // --------------------------------------------------------------------
    public override void TakeDamage(Vector3 position, Vector3 force, int damage, Rigidbody bodyPart, int hitDirection = 0)
    {
        /*
        ...
        */
        ...
        Vector3 attackerLocPos = transform.InverseTransformPoint(GameObject.Find("Player").transform.position);
        ...
        if (!shouldRagdoll)
        {
            ...
            if (hitDirection == 0)
            {
                ...
                angle = HDK_AIState.FindSignedAngle(vecToHit, transform.forward);
            }
